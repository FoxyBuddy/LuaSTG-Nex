...
CMake 配置：原项目为Visual Studio 2022，推荐改工具为VS2026.  

```shell
cmake --preset vs2026-msvc-amd64
```
之后可以自行选择使用Visual Studio或cmake和workflow

编译：  

```shell
cmake --build --preset windows-vs2026-msvc-amd64-debug
cmake --build --preset windows-vs2026-msvc-amd64-release
```

一键发行（CMake 配置、编译发行版）：  

```shell
cmake --workflow --preset windows-vs2026-msvc-amd64-release
```
...
接下来你就可以用 Visual Studio 2026 打开 slnx 解决方案开始开发。
