...
namespace luastg
{
    ...
    bool ResourcePool::CreateSprite(const char* name, const char* texname,
                                    double x, double y, double w, double h) noexcept
    {
        ...
        try
        {
            ...
            tRes.attach(new ResourceSpriteImpl(name, p_sprite.get()));
    ...
    bool ResourcePool::CreateAnimation(const char* name, const char* texname,
                                       double x, double y, double w, double h, int n, int m, int intv) noexcept
    {
        ...
        int intv) noexcept
    {
        ...
        try {
            core::SmartReference<IResourceAnimation> tRes;
            tRes.attach(
                new ResourceAnimationImpl(name, sprite_list, intv)
            );
            m_AnimationPool.emplace(name, tRes);
        }
        catch (std::exception const& e)
