...
namespace luastg::binding
{
	...
	struct ResourceCollection
	{
		...
		static int api_createSprite(lua_State* L)
		{
			...
			if (!self->data->CreateSprite(sprite_name.data(), texture->GetResName().data(), x, y, width, height))
			{
		...
		static int api_createSpriteSequence(lua_State* L)
		{
			...
			if (S.is_string(3) || S.is_userdata(3)) {
				...
				if (!self->data->CreateAnimation(sprite_sequence_name.data(), sprite_list, interval))
				{
